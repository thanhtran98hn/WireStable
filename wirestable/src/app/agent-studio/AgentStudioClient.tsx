...
export default function AgentStudioPage() {
  ...
  // States
  const [playgroundPrompt, setPlaygroundPrompt] = useState("Transfer 5 USDC to 0xa2b2... and sweep remaining to USYC");
  const [playgroundLoading, setPlaygroundLoading] = useState(false);
  const [playgroundOutput, setPlaygroundOutput] = useState<any>({
    "status": "parsed",
    "intent": "transfer",
    "entities": {
      "token": "USDC",
      "amount": "5.00",
      "recipient": "0xa2b22b2b22b2b2b22b2b2b22b2b2b22b2b2b2b",
      "chain": "Arc_Testnet"
    },
  ...
  const [ragQuery, setRagQuery] = useState("compliance");
  ...
  const handleTestPrompt = async () => {
    ...
    const taskId = startTask("ai", "Executing LLM Agent Intent Parser", { estimatedDuration: 1800 });
    try {
        ...
        <section className="grid-responsive-2col">
          ...
          <div className="card" style={{ display: "flex", flexDirection: "column", gap: "var(--space-4)" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
              <button onClick={() => setPlaygroundPrompt("send 10 USDC to Bob on Sepolia")} className="suggestion-chip" style={{ background: "rgba(255,255,255,0.03)", cursor: "pointer" }}>Bridge 10 USDC</button>
              <button onClick={() => setPlaygroundPrompt("deploy a work escrow contract for Bob, reward 200 USDC")} className="suggestion-chip" style={{ background: "rgba(255,255,255,0.03)", cursor: "pointer" }}>Deploy Work Escrow</button>
              <button onClick={() => setPlaygroundPrompt("accrue USYC yield and lock rate swap of 100 USDC")} className="suggestion-chip" style={{ background: "rgba(255,255,255,0.03)", cursor: "pointer" }}>Yield Accumulator</button>
            </div>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "6px" }}>
              ...
              {playgroundLoading ? (
                <div style={{ flex: 1, minHeight: "180px" }}>
                  <LoadingCard className="h-full border-dashed" />
                </div>
              ) : (
                <pre
                  style={{
